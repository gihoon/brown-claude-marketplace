...
name: report-design
description: >-
  Create a DSRV-style institutional research report as an A4 PDF (JP Morgan / 미래에셋 급 기관 리서치 수준).
  Use this whenever the user asks to write/build a research report, market report, 리포트, 보고서, or 참관/현장 리포트
  in DSRV's house style. Replicates the ETHConf 2026 New York report design: cover, Table of Contents,
  Executive Summary (take-aways), chapters with "DSRV View" boxes, comparison tables, pullquotes, disclaimer
  boxes; Pretendard for Korean and Source Serif 4 (serif body) + Inter (sans headings, Wall-Street style) for
  English; auto-injected header/footer; rendered to PDF via Chrome headless with locally embedded woff2 fonts.
---

# DSRV Research Report Design

이 스킬은 DSRV의 기관용 리서치 리포트(A4 PDF) 디자인을 **그대로 재현**한다. 완성된 디자인 자산
...
- **색**: `--blue-primary:#1C60EF`(강조·제목), `--blue-deep:#1B4FD6`(표 행 헤더), `--blue-tint:#EAF1FE`(표 셀), `--blue-tint-2:#F5F8FF`(DSRV View), `--ink:#141414`(본문), `--gray-footer:#9AA0A6`.
...
- **머리말·꼬리말**: HTML 하단 `<script>`가 각 `.page`에 자동 주입 — 좌상단 D 모노그램 + 우상단 페이지번호(#555), 하단 카피라이트(우측정렬) + DSRV 로고. **표지 제외, Writers=2부터** 번호.
...
5. **본문 챕터** — `h1.chapter`/`h2.section`. 컴포넌트: 비교표(`table.compare`), 그림(`.figure`+img+`.caption`), 풀쿼트(`.pullquote`), 주석(`.note`), 그리고 **각 챕터·세션 끝에 `DSRV View` 박스**(해석·시사점).
6. **Business Contact** — 제목/이메일/DSRV 로고/URL/면책 박스(본문 폭 `left:0;right:0`, 하단에 붙임).
...
3. PDF 페이지를 이미지로 읽어 표지·목차·표·DSRV View가 깨지지 않았는지 시각 확인.
...
- DSRV 역량 노출은 절제(객관성). 면책은 "자유 공유·인용 가능, 단 출처(DSRV) 표기"로 통일.
...
- `images/` — DSRV 로고 4종 (favicon dark·light, master dark·light)
