...
| 모드 | 사용법 | 효과 |
|------|--------|------|
| **build** | `/wiki build` | 전체 클러스터별 개념 페이지 일괄 생성 |
| **page** | `/wiki page 글쓰기` | 특정 주제의 개념 페이지 1개 생성/갱신 |
| **index** | `/wiki index` | 전체 위키 목차(index.md) 생성 |
...
> [!info] 자동 생성
> 이 페이지는 `/wiki build`로 생성됨. 수동 편집 시 다음 build에서 덮어씌워짐.
```
...
`/wiki page 글쓰기` — 특정 주제의 개념 페이지 1개만 생성/갱신.
...
5. **덮어쓰기 허용**: `/wiki build` 시 기존 페이지를 갱신. 수동 편집 내용은 보존 안 됨.
